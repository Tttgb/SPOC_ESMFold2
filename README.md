...
self-contained GitHub repository with an inference script and a notebook
...
- **Inference**: `inference.py` — scores one protein dimer (CIF/NPZ) and
...
├── supplementary_table_1_structural classifier.csv  # Supp. Table 1: 49 Structural classifier features
├── supplementary_table_2_SPOC_ESMFold2.csv         # Supp. Table 2: 39 SPOC ESMFOLD features
...
Data section). `inference.py` calls `deeploc2` simply as a subprocess, so
install it into **whatever Python environment you run `inference.py` in**
...
```bash
# in the same environment used to run inference.py (GPU + PyTorch needed)
...
python inference.py --cif <dimer.cif> --npz <dimer.npz> \
...
> `.cif`. `inference.py` needs the `.npz` companion because the structural
...
> coordinates / chain assignment are absent, `inference.py` auto-derives
...
| `data/analysis/` | Plotting data for the notebook (train/test, 3 RF models, Gini-importance CSVs for Fig 4, `domain_pairs_stats.csv` for Fig 1D, pre-computed `param_sweep_aupr_results_*.csv` — the notebook's sweep cell re-runs the scan itself) |
...
> to run `inference.py` (see the install note above).
...
The hyperparameter-sweep figure is **re-run inside the notebook** (last cell):
for each classifier (SPOC ESMFOLD / Structural classifier) it repeats the
3-fold CV scan over `n_estimators` × `max_depth` × `min_samples_split` with the
same parameters and seed (`random_state=42`) as the paper's reference scanner
and emits **one heatmap per `min_samples_split` (8 heatmaps in total)**, so the
figure matches the paper exactly. Pre-computed sweep results
(`data/analysis/param_sweep_aupr_results_*.csv`) are also shipped and can be
regenerated with `scripts/param_sweep_aupr_heatmap.py`
